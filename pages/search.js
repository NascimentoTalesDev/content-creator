...
export default function Buscar(){
    ...
    const LayoutContainer = styled.section`
        ...
        align-items: center;
        margin: auto;
        background-color: ${layout ? layout.colorBackground: "#CECECE"};
        overflow: hidden;
`
    ...
    const ImageContainer = styled.div`
        margin-top: ${layout ? layout.marginTopImageContainer: 10}%;
        width: 85%;
        border-radius: 5px;
        background-color: ${layout ? layout.backgroundColorImage: "#FFFFFF"};
        padding: 10px;
        display: flex;
        flex-direction: column;
        justify-content: center;
    ...
    const NomeProduto = styled.h2`
        ...
        margin-bottom: 1rem;
        font-weight: 500;
        color: ${layout ? layout.textColorProduct : "#000000"};
    ...
    const PrecoAntigo = styled.span`
        ...
        margin-bottom: 3px;
        margin-top: -4rem;
        color: ${layout ? layout.textColorPrecoAntigo: "#000000"};
    `
    const PrecoNovo = styled.span`
        font-size: 15px;
        background-color: ${layout ? layout.backgroundPrecoNovo : "#CECECE"};
        padding: 1px;
        border-radius: 2px;
        margin-bottom: 1px;
        font-weight: 900;
        color: ${layout ? layout.textColorPrecoNovo : "#000000"};
    `
    const LinkContainer = styled.div`
        background-color: ${layout ? layout.backgroundColorLink : "#FFFFFF"};
        min-height: 40px;
        width: 100%;
        display: flex;
        align-items: center;
    ...
    const Link = styled.span`
        ...
        font-size: 10px;
        font-weight: 500;
        color: ${layout ? layout.textColorLink : "#000000"};
    `
    ...
    const Message = styled.p`
        ...
        color: ${layout ? layout.textColorAviso : "#000000"};
    `

    return(
        <Layout>
            <div className="flex relative justify-around items-center h-full w-full flex-row w-28">
                {isLoading && (
                    <div className="flex flex-col absolute h-full w-full z-50 items-center justify-center bg-gray-500">
                        <p className="mb-2">Carregando layouts...</p>
                        <Spinner />
                    </div>
                )}
                <div>
